...
function startGame(){
  boomCreate = setInterval(() => {
    let left = Math.random() * (gameMain.value.getBoundingClientRect().width - 50);
  ...
  boomMove = setInterval(() => {
    ...
    boomStyle.value.forEach((item) => {
      ...
      if(item.position.top > gameMainHeight - 2 * enzymeHeight
...
function keyMove(event){
    ...
    switch(event.key){
      case "ArrowRight":{
          if(enzyme.value.getBoundingClientRect().right > gameMain.value.getBoundingClientRect().right - 10)break;
          position.left += 15;
          enzymeStyle.value.transform = `translateX(${position.left}px)`;
          break;
      }
      case "ArrowLeft":{
        if(enzyme.value.getBoundingClientRect().left < gameMain.value.getBoundingClientRect().left + 10)break;
        position.left -= 15;
        enzymeStyle.value.transform = `translateX(${position.left}px)`;
        break;
      }
